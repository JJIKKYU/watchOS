...
public struct HiraganaWritingView: View {
    @State private var drawnImage: UIImage?
    @State private var text: String?
    public let store: StoreOf<HiraganaWritingFeature>
    ...
    public init(store: StoreOf<HiraganaWritingFeature>) {
        self.store = store
    }
...
#Preview {
    let store = Store(
        initialState: HiraganaWritingFeature.State()
    ) {
        HiraganaWritingFeature()
    }
    return HiraganaWritingView(
        store: store
    )
}
